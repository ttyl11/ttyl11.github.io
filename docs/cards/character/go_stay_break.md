...
- **目标**: 单个敌方
- **可升级**: 是（1 级）
- **关键词**: [凝驻](/mechanics/ningzhu.md)（打出后留在弃牌堆，不洗回抽牌堆）
...
- **凝驻行为**：战斗结束后此牌留在[弃牌堆](/mechanics/piles.md)中，不参与洗入抽牌堆。需主动回收或消除才能重新进入牌组循环。
...
- **凝驻的经济性**：1 PP 冷却，[凝驻](/mechanics/ningzhu.md)使此牌在弃牌堆中沉淀，每场战斗仅能打一次，但可随时在弃牌堆中找到并使用（无需从抽牌堆抽到）。适合用于关键回合的斩杀或贴曝。
- **与一念生死叹的区别**：两者均为生命差增伤的[凝驻](/mechanics/ningzhu.md)攻击牌，但方向相反——[一念生死叹](./life_death_sigh.md)在自身 HP 高于目标时增伤并触发[饮血](/mechanics/blood-suck.md)回复，一念去留断则在自身 HP 低于目标时增伤并施加[曝](/powers/expose_power.md)。根据当前血量态势选择使用：高血时用生死叹（续航），低血时用过留断（爆发+控制）。
...
- [弃牌堆](/mechanics/piles.md)（凝驻卡牌的存放位置）
