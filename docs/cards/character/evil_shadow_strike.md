...
- **基础伤害**：造成 5 点攻击伤害，无连击。
- **固定伤害施加**：伤害结算后，对目标施加[固定伤害](/powers/fixed_damage_power.md)能力，数值 = `(target.MaxHp - target.CurrentHp) × percent`，其中 `percent` 为 6%（升级后 10%）。固定伤害在目标下回合开始时结算并移除，可绕过格挡。
- **固定伤害叠层**：同一目标多次被打上固定伤害时，每层独立计算并分别在下回合结算。

## 小贴士

- **已损生命即伤害**：固定伤害基数 = 目标已损生命（MaxHp - CurrentHp）。目标血量越低，已损生命越大，固定伤害越高——但注意基础伤害始终是固定的 5 点，只有固定伤害部分随目标血量变化。
- **固定伤害绕格挡**：[固定伤害](/powers/fixed_damage_power.md)以能力形式施加在目标身上，下回合开始时结算，完全不经过格挡计算，是对付高格挡敌人的有效手段。
- **10 PP 的持久战价值**：10 PP 意味着最多可打 10 次，每次叠加新的固定伤害层数。对同一目标连续施加时，层数叠加——每层独立结算，形成持续的延迟伤害压力。
